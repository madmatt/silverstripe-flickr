...
class FlickrServiceTest extends SapphireTest
{
    ...
    public function testGetSetApiKey()
    {
        ...
        $testApiKey = 'f8889db1db85cc49e3087dc1337ba6ae';   // requires a valid API Key
        $service->setApiKey($testApiKey);

        $response = $service->getApiKey();

        $this->assertEquals($response, 'f8889db1db85cc49e3087dc1337ba6ae');
    }
    ...
    public function testGetPhotosetsForUser()
    {
        ...
        $mockResponse = new Response(200, [], $body, '1.1');
    ...
    /**
     * Setup a mock service using Phake::mock() to mock a basic version of `isAPIAvaiable`
     * @param boolean $available Can change this to account for when API is unavailable
     * @return FlickrService_PHAKE
     */
    public function getMockService($available = true)
    {
        // setup mock
        $service = Phake::mock(FlickrService::class);
        $service->setApiKey('f8889db1db85cc49e3087dc1337ba6ae');
